...
graph_settings={
     {
     name="exec ",
     arg="radeontop -d- -l1 | grep -o 'gpu [0-9]\\{1,3\\}' | cut -c 5-7 ",
     max=100,
     width=430,
     height=85,
     nb_values=120,
     ...
     y=160,
     fg_bd_size=0,
     fg_bd_colour = { {0,0x587B5A,1}, {1,0xB10000,1}},
     fg_colour = { {0,0xFFFFFF,1}, {1,0x000000,0}},
     fg_orientation = "ee",
     fg_bd_orientation = "ss",
     background=false,
     foreground=true,
     inverse=false,
     },
...
function draw_gauge_ring(data)
	...
	while i <= val do
        if returned == 1 then
		    start_arc = 90 -(graph_unit_angle * i) - graph_unit_thickness
		    stop_arc = 90 - (graph_unit_angle * i)
        else
...
function conky_main(color, theme, n_cpu, swap, clock_theme, player, player_theme)
	...
	cr = cairo_create(cs)
	line = cairo_create(cs)
	lineBold = cairo_create(cs)
	...
	for i=7,n_cpu do
		...
		settings = {--CPU GRAPH CPU1
			...
			graduation_thickness=0     , graduation_mark_thickness=1 ,
			graduation_unit_angle=7   ,
			graduation_fg_colour=theme , graduation_fg_alpha=0.8     ,
			caption=''                 ,
			caption_weight=1           , caption_size=10.0           ,
			caption_fg_colour=fgc      , caption_fg_alpha=fga        ,
            returned=1,
...
function conky_mai(color, theme, n_cpu, swap, clock_theme, player, player_theme)
	...
	cairo_set_line_cap (lineBold, CAIRO_LINE_CAP_BUTT)
	cairo_set_line_width (lineBold, 20)
	cairo_set_source_rgba (lineBold, rgb_to_r_g_b(0xFFFFFF,0.1))
    cairo_rectangle (lineBold, 20, 175, 420, 15)
	cairo_stroke (lineBold)
		settings = {--CPU GRAPH CPU1
			value=tonumber(conky_parse("${exec radeontop -d- -l1 | grep -o 'gpu [0-9]\\{1,3\\}' | cut -c 5-7 }")),
			value_max=100              ,
			x=80                       , y=0                        ,
			graph_radius=50            ,
			graph_thickness=20        ,
			graph_start_angle=90     ,
			graph_unit_angle=1.8      , graph_unit_thickness=1.9   ,
			graph_bg_colour=bgc        , graph_bg_alpha=bga          ,
			graph_fg_colour=fgc        , graph_fg_alpha=fga          ,
			hand_fg_colour=fgc         , hand_fg_alpha=0.0           ,
			txt_radius=35              ,
